...
def G_main(
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    if mapping_network is not None:
        # print("got whole network")
        components.mapping = mapping_network
    elif 'mapping' not in components:
...
def D_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    with tf.variable_scope('Output'):
        x = apply_bias_act(dense_layer(x, fmaps=1))  #fmaps=max(labels_in.shape[1], 1)))
