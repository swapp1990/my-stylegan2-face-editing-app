<template>
  <div class="mixwrapper">
    ...
    <div class="item1 thinScroll">
      <checkbox-picker
        :initMax="5"
        v-on:checkedInit="onCheckedInit"
        v-on:checked="onMixLayerPicked"
      ></checkbox-picker>
    </div>
    ...
    <div class="item3">
      <div class="bar">
        <button
          ...
          class="lockCls"
          v-b-tooltip.hover
          :title="isLocked?'Locked Current Style':'Click to lock this style'"
        >
        ...
        <button
          type="button"
          class="lockCls loading"
          v-b-tooltip.hover
          :title="isMixing?'Mixing':'Completed'"
        >
...
export default {
  ...
  watch: {
    mainFaceImg: {
      handler: function (n, o) {
        if (this.isMixing && !this.isLoading) {
          this.isMixing = false;
        }
      },
      deep: true,
  ...
  methods: {
    ...mapActions("socketStore", ["sendEditAction"]),
    clear(flag) {},
    loadGallery(imgArr) {
    ...
    onMixLayerPicked(vals) {
      this.layersMixMap = vals;
      let params = {
        styleImgIdx: this.selectedImgIdx,
        layersMixMap: this.layersMixMap,
      };
      let msg = {};
      msg.action = "mixStyleImg";
      msg.params = params;
