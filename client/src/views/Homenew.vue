<template>
  <div class="wrapper">
    <b-modal id="login-modal" hide-footer title="Login">
      <div>Please enter user name or create a new one</div>username:
        ...
        <div class="panel-btm">
          <ul class="list list--buttons">
            <li v-for="attrTab in attributeTabs">
              <button
                class="tab__btn"
                :class="isAttrTabSelected(attrTab)"
                @click="changeSelectedTab(attrTab)"
                v-b-tooltip.hover
                :title="attrTab.hoverText"
              >
                <i class="fas" v-bind:class="attrTab.icon"></i>
              </button>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions("socketStore", ["connectServer", "sendEditAction"]),
    onGalleryImageClicked(params) {
      let elementIdx = this.galleryImgs.findIndex(
        (img) => img.idx == params.idx
      );
...
.thinScroll::-webkit-scrollbar-thumb {
  -webkit-box-shadow: inset 0 0 6px rgba(0, 0, 0, 0.3);
  background-color: rgba($red, 0.55);
}
.wrapper {
  display: flex;
  flex-direction: row;
