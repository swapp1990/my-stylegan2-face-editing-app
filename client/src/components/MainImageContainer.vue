      ...
      <div class="img-overlay-menu">
        <div class="menu-top">
          <button
            type="button"
            @click="saveLatent"
            :disabled="isSaved||isMainImgLoading != 0"
            v-b-tooltip.hover
          ...
          <button
            ...
            @click="randomize"
            v-b-tooltip.hover
            :disabled="isMainImgLoading != 0"
            title="Randomize/Reset all"
          >
        ...
        <div class="menu-side left" :class="{compact:!showStyleMix}">
          ...
          <div v-if="showStyleMix" class="expanded-menu">
            ...
            <div class="em-btm"></div>
          </div>
        </div>
        <div class="menu-btm">
          <comp-loading :isLoadingFlag="isMainImgLoading"></comp-loading>
          <rangeslider
            class="menu-slider"
            id="menuSlider"
            :initVal="currAttrVal"
            ...
            :max="15"
            :disabled="isMainImgLoading != 0"
            @changedAttr="onCoeffChange"
          ></rangeslider>
          <button type="button" @click="showGallery" v-b-tooltip.hover title="Go to Gallery">
    ...
    <div class="attrs-selector">
      ...
      <ul class="list list--buttons">
        <li v-for="attr in filteredAttr">
          <button
            class="list__btn"
            :class="isAttrSelected(attr)"
            @click="changeSelectedAttr(attr)"
            v-b-tooltip.hover
            :title="attr.hoverText"
          >
...
export default {
  ...
  computed: mapState({
    ...
    isMainImgLoading: (state) => state.socketStore.isMainImgLoading,
    mainFaceImg: (state) => state.socketStore.mainFaceImg,
    isRestError: (state) => state.socketStore.isRestError,
    cleared: (state) => state.socketStore.cleared,
    receiveGalleryAfterSave: (state) =>
      state.socketStore.receiveGalleryAfterSave,
  }),
  watch: {
    mainFaceImg: {
    ...
    return {
      ...
      attributes: [
        ...
        },
        {
          ...
          hoverText: "face orientation right -> left",
        },
        {
          name: "angle_pitch",
          coeff: 0.0,
          ...
          hoverText: "face orientation up -> down",
        },
        {
          name: "face_shape",
          coeff: 0.0,
          tabTag: "structure",
          icon: "fas fa-weight",
          hoverText: "face shape slant -> rounded",
        },
        {
          name: "height",
          coeff: 0.0,
          ...
          hoverText: "face shape short -> elongated",
        },
        {
          name: "width",
          coeff: 0.0,
  ...
  methods: {
    ...mapActions("socketStore", ["sendEditAction", "clearStore"]),
    ...mapMutations("socketStore", ["setIsMainImgLoading"]),
    serverConnected(flag) {
      console.log("server is connected");
      // this.isImgLoading = true;
      this.setIsMainImgLoading(1);
    },
    clear(flag) {
